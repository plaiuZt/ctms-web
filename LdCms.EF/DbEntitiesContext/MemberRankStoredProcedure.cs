...
namespace LdCms.EF.DbEntitiesContext
{
    ...
    public partial class LdCmsDbEntitiesContext
    {
        ...
        public ArrayList SP_Get_Member_RankByMaxPoints(int systemId, string companyId, int maxPoints, out int errorCode, out string errorMsg)
        {
            try
            {
                string cmdText = "SP_Get_Member_RankByMaxPoints";
                SqlParameter[] param = new SqlParameter[]
                {
                    new SqlParameter("@systemId",SqlDbType.Int,6),
                    new SqlParameter("@companyId", SqlDbType.VarChar,20),
                    new SqlParameter("@maxPoints", SqlDbType.Int,8),
                    new SqlParameter("@errorCode", SqlDbType.Int,6),
                    new SqlParameter("@errorMsg", SqlDbType.NVarChar,400)
                };
                param[0].Value = systemId;
                param[1].Value = companyId;
                param[2].Value = maxPoints;
                param[3].Direction = ParameterDirection.Output;
                param[4].Direction = ParameterDirection.Output;
                var result = this.ExecuteReaderPro(cmdText, param);
                errorCode = (int)param[3].Value;
