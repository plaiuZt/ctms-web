...
namespace CTMS.Service.Institution
{
    ...
    /// 
    /// </summary>
    public partial class StaffService:BaseService<Institution_Staff>,IStaffService
    {
        ...
        public bool SaveStaffPro(Institution_Staff entity)
        {
            try
            {
        ...
        public bool UpdateStaffPro(Institution_Staff entity)
        {
            try
            {
                int systemId = entity.SystemID;
        ...
        public bool SaveStaffPro(int systemId, string companyId, string staffId, string staffName, string username, string password, string nickname, string headImgUrl, string name, int sex, DateTime birthDate, string birthPlace, string Identification, string education, string phone, string qq, string weixin, string email, string address, decimal wages, int probation, DateTime startWorkDate, DateTime endWorkDate, DateTime signContractDate, DateTime expirationContractDate, string departmentId, string positionId, string storeId, string warehouseId, string Description, bool State)
        {
            try
            {
                int errCode = -1;
                string errMsg = "fail";
                var result = CTMSContext.SP_Add_Institution_Staff(systemId, companyId, staffId, staffName, username, password, nickname, headImgUrl, name, sex, birthDate, birthPlace, Identification, education, phone, qq, weixin, email, address, wages, probation, startWorkDate, endWorkDate, signContractDate, expirationContractDate, departmentId, positionId, storeId, warehouseId, Description, State, out errCode, out errMsg);
                if (errCode != 0)
        ...
        public bool UpdateStaffPro(int systemId, string companyId, string staffId, string staffName, string username, string password, string nickname, string headImgUrl, string name, int sex, DateTime birthDate, string birthPlace, string Identification, string education, string phone, string qq, string weixin, string email, string address, decimal wages, int probation, DateTime startWorkDate, DateTime endWorkDate, DateTime signContractDate, DateTime expirationContractDate, string departmentId, string positionId, string storeId, string warehouseId, string Description, bool State)
        {
            try
            {
                int errCode = -1;
                string errMsg = "fail";
                var result = CTMSContext.SP_Update_Institution_Staff(systemId, companyId, staffId, staffName, username, password, nickname, headImgUrl, name, sex, birthDate, birthPlace, Identification, education, phone, qq, weixin, email, address, wages, probation, startWorkDate, endWorkDate, signContractDate, expirationContractDate, departmentId, positionId, storeId, warehouseId, Description, State, out errCode, out errMsg);
                if (errCode != 0)
                    throw new Exception(errMsg);
                return errCode == 0 ? true : false;
            }
        ...
        public bool UpdateStaffStatePro(int systemId, string companyId, string staffId, bool state)
        {
            try
            {
                int errCode = -1;
                string errMsg = "fail";
                var result = CTMSContext.SP_Update_Institution_StaffState(systemId, companyId, staffId, state, out errCode, out errMsg);
                if (errCode != 0)
                    throw new Exception(errMsg);
                return errCode == 0 ? true : false;
            }
        ...
        public bool UpdateStaffPasswordPro(int systemId, string companyId, string staffId, string password)
        {
            try
            {
                int errCode = -1;
                string errMsg = "fail";
                var result = CTMSContext.SP_Update_Institution_StaffPassword(systemId, companyId, staffId, password, out errCode, out errMsg);
                if (errCode != 0)
                    throw new Exception(errMsg);
                return errCode == 0 ? true : false;
            }
        ...
        public Institution_Staff GetStaffPro(int systemId, string companyId, string staffId)
        {
            try
            {
                if (systemId == 0)
                ...
                int errCode = -1;
                string errMsg = "fail";
                var result = CTMSContext.SP_Get_Institution_Staff(systemId, companyId, staffId, out errCode, out errMsg);
                if (errCode != 0)
                    throw new Exception(errMsg);
                if (result == null)
                    return null;
                else
                    return result.ToObject<List<Institution_Staff>>().FirstOrDefault();
            }
            catch (Exception ex)
            {
                throw new Exception(ex.Message);
        ...
        public List<Institution_Staff> GetStaffPagingPro(int systemId, string companyId, int pageId, int pageSize, out int rowCount)
        {
            try
            {
                int errCode = -1;
                string errMsg = "fail";
                var result = CTMSContext.SP_Get_Institution_StaffPaging(systemId, companyId, pageId, pageSize, out errCode, out errMsg, out rowCount);
                if (errCode != 0)
                    throw new Exception(errMsg);
                if (result == null)
                    return null;
                else
                    return result.ToObject<List<Institution_Staff>>();
            }
            catch (Exception ex)
            {
                throw new Exception(ex.Message);
        ...
        public List<Institution_Staff> SearchStaffPro(int systemId, string companyId, string startTime, string endTime, string departmentId, string positionId, string storeId, string warehouseId, string keyword)
        {
            try
            {
                int errCode = -1;
                string errMsg = "fail";
                var result = CTMSContext.SP_Search_Institution_Staff(systemId, companyId, startTime, endTime, departmentId, positionId, storeId, warehouseId, keyword, out errCode, out errMsg);
                if (errCode != 0)
                    throw new Exception(errMsg);
                if (result == null)
                    return null;
        ...
        public bool VerifyStaffLoginPro(int systemId, string companyId, string username, string password)
        {
            try
            {
                int errCode = -1;
                string errMsg = "fail";
                var result = CTMSContext.SP_Verify_Institution_StaffLogin(systemId, companyId, username, password, out errCode, out errMsg);
                if (errCode != 0)
                    throw new Exception(errMsg);
                return errCode == 0 ? true : false;
            }
        ...
        public V_Institution_Staff GetVStaffPro(int systemId, string companyId, string staffId)
        {
            try
            {
                ...
                int errCode = -1;
                string errMsg = "fail";
                var result = CTMSContext.SP_Get_VInstitution_Staff(systemId, companyId, staffId, out errCode, out errMsg);
                if (errCode != 0)
                    throw new Exception(errMsg);
                if (result == null)
                    return null;
        ...
        public List<V_Institution_Staff> GetVStaffAllPro(int systemId, string companyId)
        {
            try
            {
                ...
                int errCode = -1;
                string errMsg = "fail";
                var result = CTMSContext.SP_Get_VInstitution_StaffAll(systemId, companyId, out errCode, out errMsg);
                if (errCode != 0)
                    throw new Exception(errMsg);
                if (result == null)
                    return null;
